...
# --- Fallback tokenizer: if NLTK data (punkt/punkt_tab) is missing, use regex ---
def _simple_tok(text: str):
...
def compute_freq(sentence, n=2):
    tokens = _tok(sentence)
    ngrams = nltk.ngrams(tokens, n)
    return nltk.FreqDist(ngrams)


def tfidf_similarity(text_a, text_b, vec):
    # 稀疏 TF-IDF：不要 densify
    encs = vec.transform([text_a, text_b]).tocsr()  # (2, V)
    v0, v1 = encs[0], encs[1]
    num = v0.multiply(v1).sum()
    den = (v0.multiply(v0).sum() ** 0.5) * (v1.multiply(v1).sum() ** 0.5)
    if den == 0:
        return 0.0
