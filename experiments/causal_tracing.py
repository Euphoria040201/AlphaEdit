...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        #]
        pat = re.compile(r"^(transformer|gpt_neox|model)\.(h|layers)\.\d+$")
        self.layer_names = [n for n, m in model.named_modules() if pat.match(n)]
        self.num_layers = len(self.layer_names)
        # 兜底（有些实现只在 model.model.layers 里）
        if self.num_layers == 0 and hasattr(model, "model") and hasattr(model.model, "layers"):
